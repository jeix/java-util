...
## 13. pipe 의 두 구현과 null 검사 시점 (2026-09-15)

`pipe(Function...)` 은 코드릿에 있던 두 구현을 그대로 살려 무작위로 분기한다(결정 3과 같은 방식).

- `_pipeByApply` — 입력값에 함수를 차례로 적용한다. 붙일 때는 아무것도 하지 않는다.
- `_pipeByCompose` — `Function::andThen` 으로 미리 합성해 둔다.

**null 함수 검사를 붙이는 자리로 앞당겼다.** 함수 목록에 `null` 이 섞여 있으면 합성 방식은
`pipe(...)` 를 부르는 순간에, 적용 방식은 `apply()` 할 때 NPE 가 난다. 그냥 두면 두 구현의 실패
시점이 갈려서 무작위 분기가 관찰 가능해진다. 그래서 목록에 옮겨 담으면서 `requireNonNull` 한다.

**가변인자 배열을 그대로 다른 메서드에 넘기지 않는다.** `Stream.of(fns)` 나 `Arrays.stream(fns)` 로
넘기면 javac 이 `[varargs]` heap pollution 경고를 낸다. `List` 에 옮겨 담아 넘긴다
(`CollectionUtil` 에서도 같은 이유로 같은 방식을 썼다).

`pipe()` (인자 없음)는 `Pipeline` 빌더를 돌려준다. `then` 은 앞의 파이프라인을 그대로 두고 새로
만들어 돌려주므로, 중간 단계를 붙잡아 두고 다시 쓸 수 있다. `Pipeline.init()` 은 코드릿에 있던
이름을 그대로 썼다(이 저장소의 "비공개 메서드에 `_`" 규칙과는 어긋나지만, 준 코드를 살렸다).
